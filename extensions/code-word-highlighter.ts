...
const REACT_HOOKS = /\b(useState|useEffect|useCallback|useMemo|useRef|useReducer|useContext|useLayoutEffect|useImperativeHandle|useTransition|useDeferredValue|useId|useSyncExternalStore|useInsertionEffect)\b/g;
...
function extraHighlight(line: string, lang: string | undefined, theme: Theme): string {
	switch (lang) {
		case "typescript":
		case "javascript":
		...
		case "html":
		case "xml":
			return line.replace(HTML_TAG, (_match, prefix: string, tag: string, suffix: string) => {
...
function renderHighlightedCode(code: string, lang: string | undefined, theme: Theme): string[] {
	return highlightCode(code, lang).map((line) => extraHighlight(line, lang, theme));
}
...
export default function codeWordHighlighter(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		renderResult(result, { expanded, isPartial }, theme: Theme) {
			...
			if (content?.type !== "text") return new Text(theme.fg("error", "No text content"), 0, 0);
